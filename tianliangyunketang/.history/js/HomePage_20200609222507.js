...
$.ajax({
    ...
    success: function (res) {
        ...
        for (let i = 0; i < boutique.length; i++) {
            $(".boutique1 img").attr("src", res.rows[i].bannerFileUrl);
            $(".boutique1 .dt2").text(res.rows[i].courseTitle)
            $(".boutique1 .span1").text(res.rows[i].subSectionNum)
            $(".boutique1 .span2").text(res.rows[i].participationsCount)
            console.log(res.rows[i].isFree)
            if (res.rows[i].isFree == 1) {
                $(".boutique1 .span3").text("免费");
                $(".boutique1 .box_0").attr("style", "display: none")
            } else if (res.rows[i].isFree == 0) {
                $(".boutique1 .span2").text(res.rows[i].coursePrice)
                $(".boutique1 .span2").text(res.rows[i].discountPrice)
                $(".boutique1 .box2").text("限时钜惠");
            }
        }
