...
$.ajax({
    ...
    success: function (res) {
        console.log(res)
        $(".boutique1 img").attr("src", res.rows[9].bannerFileUrl);
        $(".boutique1 .dt2").text(res.rows[9].courseTitle)
        $(".boutique1 .span1").text(res.rows[9].subSectionNum)
        $(".boutique1 .span2").text(res.rows[9].participationsCount)
        var boutique = document.getElementsByClassName("boutique");
        // console.log(boutique);
        // for (var i = 0; i < boutique.length; i++) {
        //     console.log(boutique[i].attr("src", res.rows[9].bannerFileUrl))
        // }
