...
$.ajax({
    ...
    success: function (res) {
        ...
        for (let i = 0; i < boutique.length; i++) {
            console.log(i)
            $(".boutique img")[i].attr("src", res.rows[i].bannerFileUrl);
            $(".boutique .dt2")[i].text(res.rows[i].courseTitle)
            $(".boutique .span1")[i].text(res.rows[i].subSectionNum)
            $(".boutique .span2")[i].text(res.rows[i].participationsCount)
            // console.log(res.rows[i].isFree)
            if (res.rows[i].isFree == 1) {
                $(".boutique .span3")[i].text("免费");
                $(".boutique .box_0")[i].attr("style", "display: none")
            } else if (res.rows[i].isFree == 0) {
                $(".boutique .span2")[i].text(res.rows[i].coursePrice)
                $(".boutique .span2")[i].text(res.rows[i].discountPrice)
                $(".boutique .box2")[i].text("限时钜惠");
            }
        }
