...
$.ajax({
    ...
    success: function (res) {
        ...
        for (let i = 0; i < res.data.sections.length; i++) {
            var nm1 = "<div class='box_1'><h3></h3><dt></dt></div>"
            $('#directory').append(nm1)
            $($('.box_1 h3')[i]).html(i + 1 + ".&nbsp;&nbsp;" + res.data.sections[i].sectionName + 'button><svg class="icon" aria-hidden="true"><use xlink:href="#icon-ziyuan"></use></svg> 下载</button>')

            /* 目录子标题 */
            for (let j = 0; j < res.data.sections[i].subSections.length; j++) {

                $($('#directory dt')[i]).append('<dl class="dlBox">' + iconfont + knob + '</dl>');
                $($('.box_1')[i]).find('.leftBox').eq(j).html((i + 1) + '-' + (j + 1) + '&nbsp;&nbsp;' + res.data.sections[i].subSections[j].sectionName);
            }
